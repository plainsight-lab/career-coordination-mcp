...
// - payload_hash binds this override to a specific artifact:
//   payload_hash == stable_hash64_hex(envelope.artifact_id)
//   The binding is verified by ValidationEngine::validate() before applying the override.
//   Override is applied only if both rule_id matches a BLOCK finding AND payload_hash matches.
